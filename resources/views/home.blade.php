        ...
        <form method="POST" action="{{route('store')}}" enctype="multipart/form-data">
            ...
            <div class="lg:flex lg:justify-between">
                <div class="w-full lg:w-1/2 mb-6 mr-2">
                    <label class="block mb-2 uppercase font-bold text-xs text-gray-700" for="nip">
                        Nip
                    </label>

                    <input class="border border-gray-400 p-2 w-full" type="text" name="nip" id="nip"
            ...
            <div class="mb-6">
                ...
                <input class="border border-gray-400 p-2 w-full" type="email" name="email" id="email"
                    placeholder="sugi@example.com" required>
                @error('email')
                <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="lg:flex">
                ...
                <div class="w-full lg:w-1/2 mb-6">
                    ...
                    <input class="border border-gray-400 p-2 w-full" type="text" name="mobile_no" id="mobile_no"
                        placeholder="+62838xxxxxxxx" required>
                    @error('mobile_no')
                    <p class="text-red-500 text-xs mt-2">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                <textarea class="border border-gray-400 p-2 w-2/3 p-4" rows="5" name="current_address"
                    id="current_address" placeholder="Jl Kenangan No.99 Brebes"
                    required></textarea>
                            ...
                            <tr>
                                    ...
                                    <div class="flex items-center">
                                        <div class="flex-shrink-0 h-10 w-10">
                                            <img class="h-10 w-10 rounded-full"
                                                src="{{asset('pegawais/images/'.$pegawai->image)}}" alt="">
                                        </div>
                                        <div class="ml-4">
                                ...
                                <td
                                    ...
                                    <a class="cursor-pointer text-indigo-600 hover:text-indigo-900 mx-2"
                                        onclick="document.querySelector('#modal-view-{{$pegawai->id}}').classList.remove('hidden');document.querySelector('#modal-view-{{$pegawai->id}}').classList.add('fixed');">View</a>
                                    <a class="text-indigo-600 hover:text-indigo-900 mx-2 cursor-pointer"
                                        onclick="document.querySelector('#modal-register-{{$pegawai->id}}').classList.remove('hidden');document.querySelector('#modal-register-{{$pegawai->id}}').classList.add('fixed');">Edit</a>
                                    <a href="{{route('print-id', $pegawai->id)}}" target="_blank"
                                        class="text-indigo-600 hover:text-indigo-900 mx-2">Print</a>
                                    <form method="POST" action="{{route('delete', $pegawai->id)}}"
                                        id="delete-{{$pegawai->id}}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                                            onclick="if(confirm('Do you want to delete?'))document.getElementById('delete-{{$pegawai->id}}').submit()"
                                            class="cursor-pointer text-red-600 hover:text-red-900 mx-2">Delete</button>
                                    </form>
                                </td>
                            </tr>
                            @include('view-pegawai')
                            @include('edit-pegawai')
                            @endforeach
                        </tbody>
                    </table>
                    <div class="p-2">
